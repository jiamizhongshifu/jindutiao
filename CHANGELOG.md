...
- **弹幕系统** 🎬
  - Bilibili风格滚动弹幕，让时间管理更有趣
  - 11种任务分类智能匹配(工作/学习/休息/运动/会议/用餐/通勤/娱乐/睡眠/励志/默认)
  - 自动任务识别，根据当前任务显示相应弹幕
  - 智能配色系统，不同类别自动使用不同颜色
  - 60fps流畅动画，电影级流畅体验
  - 130+预设弹幕内容，纯文本无emoji
  - 可配置参数(频率/速度/字体/透明度/最大数量/位置)

### 🐛 Bug修复 / Bug Fixes

- **鼠标悬停检测修复**
  - 修复鼠标在弹幕区域触发任务提示的问题
  - 修复鼠标在标记图片区域触发任务提示的问题
  - 优化任务提示仅在进度条区域内显示
...
- PyInstaller资源路径适配
- 独立定时器管理，不影响其他功能性能
- 完整的弹幕生命周期管理(生成/移动/清理)

---

## [v1.6.8] - 2025-12-02

### 💳 支付系统修复 / Payment System Fixes

- **支付自动识别完整修复**
  - 绕过 Vercel 缓存，直接调用 manual_upgrade API 验证支付状态
  - manual_upgrade API 添加 Z-Pay 真实支付验证，防止未支付激活
  - 实现 webhook 回调机制替代轮询方式
  - 支付成功自动激活会员，无需手动刷新

- **数据库架构修复**
  - 添加 users.tier 和 subscription_expires_at 字段
  - 创建 upgrade_user_subscription RPC 函数绕过 PostgREST schema cache
  - 添加 payment_cache 表支持 webhook 回调缓存
  - RPC Fallback 逻辑确保兼容性

### 🐛 Bug修复 / Bug Fixes

- **会员状态查询修复**
  - 修复字段名不一致(tier vs user_tier)导致的会员降级问题
...
### 💰 定价调整 / Pricing Updates

- **恢复正常价格**
  - Pro月度: ¥29/月（原测试价 ¥0.1）
  - Pro年度: ¥199/年（原测试价 ¥0.1）
  - 终身会员: ¥599（原测试价 ¥0.1，限量1000名）
...
- **场景编辑器完全整合**
  - 移除独立的SceneEditor.exe，完全集成到主应用GaiYa-v1.6.exe
  - 删除SceneEditor.spec构建配置文件
  - 优化项目结构，减少文件冗余

- **开发流程统一**
  - 测试: 统一使用 `python main.py` 启动主程序
  - 打包: 仅需 `pyinstaller Gaiya.spec` 一次性打包
  - 移除独立场景编辑器的启动和打包流程

- **模块化集成改进**
  - scene_editor 作为 Python 模块直接导入，而非子进程调用
  - config_gui.py 中通过 SceneEditorWindow 类实例化编辑器
  - 统一资源管理：编辑器与主窗口共享 scenes/ 目录

### ✨ 新功能 / New Features

- **道路层选择同步**
  - 实现画布与素材库的双向选择联动
  - 点击画布中的道路图片，素材库自动高亮对应项
  - 提升场景编辑体验和操作效率
  - **实现位置**: `scene_editor.py:3045-3064`
...
- 🗑️ **删除**: SceneEditor.spec（不再需要独立打包配置）
- 🗑️ **删除**: dist/SceneEditor.exe（已整合到主程序）
- ✏️ **修改**: scene_editor.py（道路层选择同步功能）
- ✏️ **修改**: config_gui.py（场景编辑器窗口管理）
- ✏️ **修改**: version.py（版本号更新到 1.6.2）
- ✏️ **修改**: README.md（更新版本信息和功能说明）
...
- **场景编辑器缩放显示修复**
  - 修复100%缩放时未显示真实1:1像素比例的问题
  - 使用绝对缩放替代增量缩放,确保显示精度
  - 优化缩放体验,精确还原场景原始尺寸

- **道路层属性面板显示修复**
  - 修复导入场景后道路层属性面板不显示的问题
  - 增强show_road_panel()方法的同步机制
  - 确保场景数据与UI状态一致

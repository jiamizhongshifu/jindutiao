...
- ✅ 写代码时, 要注意**圈复杂度**, 代码尽可能复用
- ✅ 写代码时, 注意**模块设计**, 尽量使用设计模式
...
- ✅ 严禁使用 `any` 类型,所有类型必须明确定义
- ✅ 所有数据库操作必须使用类型化的模型类

**PySide6 / Qt 项目规范**:
- ✅ 图片加载必须使用专用的 `QPixmap` 或 `QIcon` 组件
- ✅ 信号槽连接必须类型安全,避免使用字符串形式
- ✅ UI 样式尽量使用 QSS (Qt Style Sheets),保持样式集中管理

**数据库操作规范**:
- ✅ 所有 Supabase 数据库操作必须使用强类型模型
- ✅ 禁止直接拼接 SQL 查询,使用参数化查询防止注入
- ✅ 所有数据库字段访问必须经过类型验证
...
**最后更新**: 2025-12-07
**配置版本**: v4.2 (更新编码规范,增强类型安全和错误处理要求)

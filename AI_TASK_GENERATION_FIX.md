...
**原因1**: `ai_client.py:82-87` 只处理 `403` 状态码,未处理 `429`
```python
# 旧代码
...
**原因2**: `ai_client.py:347-350` 在 `parent_widget=None` 时静默返回
```python
# 旧代码
...
- ✅ 429错误被正确识别和处理
- ✅ 日志清晰记录错误原因: `"Daily AI quota exceeded"`
- ✅ 即使无法显示对话框,日志也会记录警告
...
- ✅ **429错误处理** (`ai_client.py`): 下次打包exe后生效
- ❌ **任务顺序修复** (`api/plan-tasks.py`): 需要部署到Vercel

### 部署步骤

```bash
# 1. 提交代码
git add ai_client.py api/plan-tasks.py
git commit -m "fix(ai): 修复AI任务生成的两个关键问题"
...
# 测试端点
curl -X POST http://localhost:3000/api/plan-tasks \
  -H "Content-Type: application/json" \
  -d '{"user_id": "test", "input": "休息日计划", "user_tier": "pro"}'
```
